...
`robotall` 是用于控制 TonyPi 机器人和进行比赛通信的基础库。其中，`act` 用于执行机器人预置动作组，例如站立、前进、后退和转向，可用于测试机器人动作是否正常；`register_robot` 用于通过 ST25DV04 NFC 标签发送机器人的注册信息；`send_request` 用于通过该标签发送比赛任务请求，并读取返回结果。
