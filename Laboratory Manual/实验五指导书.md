...
本实验中，学生不需要直接访问 KV260，也不需要指定具体使用哪一块 FPGA。Central Server 会根据 `student_id` 自动找到该学生最新上传的 Artifact，并将计算任务分配到可用的 KV260 上执行。
...
* `student_id`：学生或小组的唯一标识，需要与上传 Artifact 时使用的 `student_id` 保持一致；
* `image`：需要进行 FPGA 分类的花卉图片，目前支持 JPEG 和 PNG 格式。

例如，假设：

```text
Central Server IP：192.168.31.254
端口：8000
...
```bash
curl -X POST http://192.168.31.254:8000/predict -F student_id=student01 -F image=@flower.jpg
```
