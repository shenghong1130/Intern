...
其中 `json=data` 表示将 Python 字典以 JSON 格式发送给服务器。
...
上传 Artifact 后，可以通过 HTTP 向 Central Server 提交 FPGA 计算任务。
...
其中 `payload` 中图片的编码方式、尺寸、通道顺序和字段名称，应以课程最终提供的 KV260 PYNQ Worker 接口为准。
...
* HTTP `200`，状态为 `completed`：FPGA 计算已经完成，可以直接读取分类结果。
* HTTP `202`，状态为 `queued`：任务正在等待执行，需要保存 `request_id`，并通过

```text
GET /requests/{request_id}
```

查询任务状态，直到任务变为 `completed`。

* 其他状态：记录 HTTP 状态码、响应内容和 `request_id`，根据服务器返回的信息处理，避免无限重复请求。
...
```text
robot_code/robot_tonypi/fpga_flower_server/fpga_server_api_ready.py
```
