...
2. 在 Windows 10/11 下（所有实验）熟悉机器人平台，可以调用动作组使得机器人完成动作，可以调用摄像头使得机器人拍摄照片和执行基础动作。

## **二、考核要求**

【考核】要求机器人从指定起点走到指定终点并转身返回起点，机器人行进不能摔倒或偏离场地（到达终点的标准为机器人两脚的中间位置不能偏离指定点 15cm 以上）。在开始整个动作前和完成整个动作后各拍一张照片并储存，之后保存到本地。
...
本节烧录的是 **TonyPi 机器人 Raspberry Pi 5 使用的 microSD 卡**，不是 KV260 的 SD 卡，请注意区分。

烧录前，请先下载以下文件和工具：

- 机器人系统镜像：[下载地址](https://cloud.tsinghua.edu.cn/d/eb55393d900349848ecc/)
- Windows 烧录工具 Win32 Disk Imager：[下载地址](https://sourceforge.net/projects/win32diskimager/)

按照以下步骤操作：

1. 下载机器人系统镜像文件。如果下载的是压缩包，先将其解压，得到 `.img` 文件。
2. 下载并打开 Win32 Disk Imager。
3. 关闭 TonyPi 电源，取出机器人 Raspberry Pi 5 使用的 microSD 卡，通过读卡器将其插入电脑。
4. 在 Win32 Disk Imager 的 **Image File** 中选择刚才解压得到的 `.img` 文件。
5. 在 **Device** 中确认并选择 microSD 卡对应的盘符。
6. 点击 **Write**，确认提示后开始烧录。
7. 等待进度完成并出现写入成功提示。
8. 在 Windows 中安全弹出 microSD 卡，然后将卡重新插入 TonyPi 的 Raspberry Pi 5。
9. 打开 TonyPi 电源。之后按照下一节的 IP、`ping` 和 SSH 流程，确认机器人能够正常启动。

> **重要：烧录前一定要确认 Device 选中的是 microSD 卡对应的盘符，不要误选电脑的其他硬盘，否则可能覆盖其中的数据。**

> **提示：** 烧录完成后，如果 Windows 提示“需要格式化磁盘才能使用”，请不要格式化，直接取消即可。机器人使用的 Linux 系统分区不一定能被 Windows 识别。
...
![alt text](实验一指导书_assets/image_01.jpg)
...
sudo shutdown now
```
![alt text](实验一指导书_assets/image_02.png)
...
![alt text](实验一指导书_assets/image_03.png)

![alt text](实验一指导书_assets/image_04.png)
...
![alt text](实验一指导书_assets/image_05.png)
...
![alt text](实验一指导书_assets/image_06.png)
...
本节使用`demo1.ipynb`。该 Notebook 将拍照、转头和行走拆分为独立单元，建议在机器人上的 Jupyter Notebook 中按从上到下的顺序运行。首次使用时，请先确认机器人已启动，并将机器人放在平整地面上。

#### **3.2.1 打开示例并完成初始化**

在 Jupyter 的文件列表中打开 `demo1.ipynb`。先运行“初始化”单元，再运行“定义拍照与转头函数”单元。初始化会加载动作组、相机和舵机控制库；只有这两个单元均正常运行后，后续测试才能执行。
