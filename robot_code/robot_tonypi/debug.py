...
function renderSummary(data){
  ...
  document.getElementById('summary').innerHTML =
    ...
    '<b>最近目标选择 / Nearest selection</b>: 距离/distance='+esc(fmt(data.current_target_distance_cm,1))+'cm, 19cm target='+esc((plan.task_target_xy||[]).join(','))+', yaw='+esc(fmt(plan.task_target_yaw_deg,1))+'°, face='+esc(plan.surface_face||'-')+', normal='+esc((plan.cardinal_normal_xy||[]).join(','))+', 剩余/remaining='+esc((data.remaining_target_ids||[]).join(','))+'<br>'+
    '<b>19cm确认与分类授权 / Confirmation & authorization</b>: arrived='+esc(data.arrived_at_target)+', confirmedTag='+esc(confirmation.tag_id||'-')+', confirmedScreen='+esc(confirmation.screen_id||'-')+', bound='+esc(confirmation.binding_ok)+', forward10cm='+esc(data.final_forward_executed)+', classifierAllowed='+esc(data.classifier_allowed)+', flower='+esc(authorization.flower||'-')+', confidence='+esc(fmt(authorization.confidence,3))+'<br>'+
...
function renderScreens(data){
  ...
  document.getElementById('screens').innerHTML =
    '<table><tr><th>屏幕 / Screen</th><th>Worker</th><th>状态 / Status</th><th>尝试 / Attempts</th><th>花朵 / Flower</th><th>置信度 / Conf</th><th>19cm唯一目标 / Target pose</th><th>目标面 / Face-normal</th><th>备注 / Notes</th></tr>'+rows+'</table>';
}
