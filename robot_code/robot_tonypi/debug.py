...
function renderSummary(data){
  ...
  document.getElementById('summary').innerHTML =
    ...
    '<b>17cm确认与分类授权 / Confirmation & authorization</b>: arrived='+esc(data.arrived_at_target)+', confirmedTag='+esc(confirmation.tag_id||'-')+', confirmedScreen='+esc(confirmation.screen_id||'-')+', bound='+esc(confirmation.binding_ok)+', forward3cm='+esc(data.final_forward_executed)+', classifierAllowed='+esc(data.classifier_allowed)+', flower='+esc(authorization.flower||'-')+', confidence='+esc(fmt(authorization.confidence,3))+'<br>'+
