...
- Formal target navigation is `POSITION A* (>10 cm) → NEAR_TARGET_ADJUSTMENT (5–10 cm) → FINAL_YAW_ALIGNMENT (real distance <=5 cm)`. Arrival uses the continuous real RobotPose, never a grid center. Near-target adjustment evaluates only one physical cycle of forward/back/left/right, requires target-owned corridor safety and strict distance reduction, then forces visual relocalization; it is bounded to four attempts. Desired yaw remains deferred until position arrival. `NavigationPlan.actions` is the authoritative A* executor input; do not re-derive motion from `path_xy`.
