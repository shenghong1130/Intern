...
After direct navigation reaches the locked 17 cm coordinate and its cardinal yaw, one live frame must contain the same 1–36 Tag and a screen quad bound to that Tag. This geometry confirmation authorizes exactly one `interaction_forward_3cm`. Classification happens from a fresh bound crop immediately after that motion; a valid non-target FPGA result creates a target-specific `VisualAuthorization`.

Physical change is exclusively:

```text
locked 17 cm Tag/screen confirmation
