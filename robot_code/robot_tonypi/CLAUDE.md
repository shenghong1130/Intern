...
`--dry-run` means no hardware. `--skip-change` performs real navigation and classification but skips the dedicated 10 cm action, arm movement, and Worker request. `--skip-api` is its deprecated alias.
...
- `target_lateral_offset_cm` and the real displacement of `interaction_forward_10cm` require field calibration.
