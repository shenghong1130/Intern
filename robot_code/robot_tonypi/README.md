...
远距离识别只更新屏幕信息，不能直接举手或发送换花请求。真实换花必须同时满足距离、身体朝向、横向读卡位置和定位有效性检查。
...
程序先定位，再扫描、绑定屏幕左侧 AprilTag、裁剪 28×28 图像并调用 FPGA 分类器。识别非目标花只会记录为 `NEEDS_CHANGE`，不会直接换花。
...
| `--mode harvest` | 定位后扫描识别，不进入完整任务导航 |
