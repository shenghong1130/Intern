...
→ 根据 Tag 四角判断西/东/南/北面
→ 生成建筑面中心正前方配置距离（默认 20 cm）、带左手横向补偿的唯一任务位姿
→ 使用现有地图、障碍代价和 A* 直接导航到配置任务坐标及标准 yaw
...
导航途中会复用原本的定位帧框取屏幕；仅当 `Tag ID == Screen ID` 且 crop 有效时调用分类器，并保存最新有效绑定结果，不会改变 Screen 状态，也不会为了缓存额外停靠或切换目标。每处理一个目标后，程序按最新 pose 到各 `task_target_xy` 的欧氏距离重新选最近目标；同距离按 screen/tag ID 升序。
...
地图、Tag 世界坐标、AprilTag 定位算法和全局障碍代价没有变化。程序不生成独立的二次对准点；`target_xy`、`interaction_xy` 与 `task_target_xy` 都指向同一个配置身体目标。最终 yaw 只能是 `0°`、`-180°`、`+90°`、`-90°`。距离锁定目标不超过 40 cm 时，程序先检查一条窄直达通道：物理占用、其他建筑和动态障碍仍会阻断，只忽略当前目标建筑自身的膨胀代价。通道安全时优先直走、短距离正后方优先倒退、再侧移；否则回到原 action planner/A*。
...
其中 `left_hand_body_offset_cm = 0` 只是未知机械尺寸的占位值。正式比赛前应现场确认左手相对身体中心的横向偏移、配置任务落点，以及专用动作组是否真实前进约 10 cm。
...
这些测试不会驱动机器人，主要检查配置任务目标、Tag/屏幕缓存授权、单次 10 cm 动作、Worker 异常收尾和屏幕左侧 AprilTag 绑定。
...
程序先定位，选择最近目标并真实导航；确认进入到达状态后，只对当前目标绑定的屏幕裁剪 28×28 图像并调用 FPGA 分类器。`harvest` 是识别模式，不执行最后 10 cm 或实体换花。机器人会移动，周围必须留出安全空间。
...
| 真实动作组和导航 | 否 | 是 |
| 真实导航到配置任务目标 | 否 | 是 |
| 需要换花时执行专用 10 cm 动作 | 否 | 否 |
| 真实举左手 | 否 | 否 |
| 真实 `send_request` | 否 | 否 |

因此 `--skip-change` 仍会让机器人导航到配置任务点并完成真实识别，但不会执行专用 10 cm 动作、举手或 Worker 请求。`--skip-api` 是它的旧兼容别名。
...
- 配置任务目标（默认 20 cm）、四向身体 yaw、横向读卡位置和左手偏移已现场验证；
- `interaction_forward_10cm` 动作实测接近 10 cm，且不会重复执行；
- 场地周围安全，操作员可以随时停止机器人。
...
```text
任务点实时确认目标 Tag，并采用 15 秒内绑定分类或有限新鲜 fallback
→ interaction_forward_10cm（恰好一次）
→ 拍照、裁剪并调用 FPGA
