...
→ 直接导航到该屏幕 target_xy
→ 到达后使用屏幕左上 AprilTag 确认 screen_id
...
导航途中会继续框取屏幕并绑定左上 AprilTag，但不会裁剪花朵、调用分类器或投票，也不会为了观察其他屏幕而停靠或切换目标。每处理一个目标后，程序按最新 pose 到各 `target_xy` 的欧氏距离重新选最近目标；同距离按 screen/tag ID 升序。`target_xy` 默认位于屏幕正前方 34 cm，是直达与到达后拍摄点，不是旧的远距离观察停靠点。真实换花还必须同时满足距离、身体朝向、横向读卡位置和定位有效性检查。
