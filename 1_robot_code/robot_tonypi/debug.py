...
const navPhaseLabel={POSITION_NAVIGATION:'POSITION A*',NEAR_TARGET_ADJUSTMENT:'NEAR TARGET',FINAL_YAW_ALIGNMENT:'FINAL YAW',ARRIVED:'ARRIVED'};
...
function renderSummary(data){
  ...
  const tagSelection=health.tag_selection||{};
  ...
  document.getElementById('summary').innerHTML =
    ...
    '<b>导航阶段 / Navigation phase</b>: '+esc(navPhaseLabel[data.current_navigation_phase]||data.current_navigation_phase||'-')+'<br>'+
    ...
    '<b>任务点确认与分类授权 / Confirmation & authorization</b>: arrived='+esc(data.arrived_at_target)+', confirmedTag='+esc(confirmation.tag_id||'-')+', confirmedScreen='+esc(confirmation.screen_id||'-')+', bound='+esc(confirmation.binding_ok)+', source='+esc(authorization.source||'-')+', cacheAge='+esc(fmt(authorization.cache_age_s,2))+'s, finalForward='+esc(data.final_forward_executed)+', flower='+esc(authorization.flower||'-')+', confidence='+esc(fmt(authorization.confidence,3))+'<br>'+
    ...
    '<b>AprilTag定位选择 / Selection</b>: selected_tag_id='+esc(tagSelection.selected_tag_id==null?'-':tagSelection.selected_tag_id)+
    ', selected_tag_type='+esc(tagSelection.selected_tag_type||'-')+
    ', fallback_to_ground='+esc(Boolean(tagSelection.fallback_to_ground))+
    ', rejection reason='+esc((tagSelection.rejected_tags||[]).map(x=>x.tag_id+':'+(x.rejection_reason||x.reason)).join(', ')||'-')+'<br>'+
    ...
    ', 转向证据/turnProgress='+esc(recovery.turn_progress_status||'-')+
    ', 已验证无进展/verifiedNoProgress='+esc(recovery.verified_no_progress_count||0)+
